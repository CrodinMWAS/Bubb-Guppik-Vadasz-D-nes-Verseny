...
function setPoint() {
    ...
    if(empty.innerHTML != "A választ húzd ide!"){
        ...
        localStorage.setItem(document.querySelector("h1").dataset.value+"ans",par);
        localStorage.setItem(document.querySelector("h1").dataset.value+"ansText",empty.innerHTML.toString());
    }
}
...
document.querySelectorAll(".answer").forEach(item => {
    ...
    if(item.dataset.value == 1){
        localStorage.setItem(document.querySelector("h1").dataset.value+"ansGood",item.innerHTML);
   }
});
